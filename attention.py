...
class Attention(nn.Module):
    ...
    def execute(self, x: jt.Var) -> jt.Var:
        ...
        attn = jt.nn.bmm_transpose(q, k, transpose_y=True)

        attn = jt.nn.softmax(attn, dim=-1)
        attn = self.attn_drop(attn)

        x = jt.nn.bmm(attn, v).transpose(1, 2).reshape(B, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class CrossAttention(nn.Module):
    ...
    def execute(self, q: jt.Var, k: jt.Var, v: jt.Var) -> jt.Var:
        ...
        attn = jt.nn.bmm_transpose(q, k, transpose_y=True) # [B, nh, N, M]

        attn = jt.nn.softmax(attn, dim=-1) # [B, nh, N, M]
        attn = self.attn_drop(attn)

        x = jt.nn.bmm(attn, v).transpose(1, 2).reshape(B, N, -1) # [B, nh, N, M] @ [B, nh, M, C/nh] --> [B, nh, N, C/nh] --> [B, N, nh, C/nh] --> [B, N, C]
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class MemEffCrossAttention(CrossAttention):
    def execute(self, q: jt.Var, k: jt.Var, v: jt.Var, attn_bias=None) -> jt.Var:
        return super().execute(q, k, v)