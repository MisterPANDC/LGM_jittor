...
from jittor.nn import ModuleList

from attention import MemEffAttention

class MVAttention(jt.nn.Module):
    def __init__(
        self, 
        dim: int,
        num_heads: int = 8,
...
class ResnetBlock(jt.nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv(in_channels, out_channels, kernel_size=3, stride=1, padding=1)

        self.norm2 = nn.GroupNorm(num_groups=groups, num_channels=out_channels, eps=eps, affine=True)
        self.conv2 = nn.Conv(out_channels, out_channels, kernel_size=3, stride=1, padding=1)

        self.act = jt.nn.silu

        self.resample = None
        if resample == 'up':
            self.resample = partial(jt.nn.interpolate, scale_factor=2.0, mode="nearest")
        elif resample == 'down':
            self.resample = nn.Pool(2, stride=2, op='mean')
        
        self.shortcut = jt.nn.Identity()
        if self.in_channels != self.out_channels:
            self.shortcut = nn.Conv(in_channels, out_channels, kernel_size=1, bias=True)
...
class DownBlock(jt.nn.Module):
    def __init__(
        ...
        if None in attns:
            ...
            self.attns = ModuleList(attns)

        self.downsample = None
        if downsample:
            self.downsample = nn.Conv(out_channels, out_channels, kernel_size=3, stride=2, padding=1)
...
class UpBlock(jt.nn.Module):
    def __init__(
        ...
        self.upsample = None
        if upsample:
            self.upsample = nn.Conv(out_channels, out_channels, kernel_size=3, stride=1, padding=1)

    def execute(self, x, xs):

        for attn, net in zip(self.attns, self.nets):
            res_x = xs[-1]
            xs = xs[:-1]
            x = jt.concat([x, res_x], dim=1)
            x = net(x)
            if attn:
                x = attn(x)
            
        if self.upsample:
            x = jt.nn.interpolate(x, scale_factor=2.0, mode='nearest')
            x = self.upsample(x)
...
class UNet(jt.nn.Module):
    def __init__(
        ...
        # first
        self.conv_in = nn.Conv(in_channels, down_channels[0], kernel_size=3, stride=1, padding=1)
        ...
        self.conv_out = nn.Conv(up_channels[-1], out_channels, kernel_size=3, stride=1, padding=1)
